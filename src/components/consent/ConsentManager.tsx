      ...
      >
        <div className="px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-6 md:p-8">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            <ConsentButton onClick={rejectAll}>{t.rejectAll}</ConsentButton>
            <ConsentButton onClick={acceptAll}>{t.acceptAll}</ConsentButton>
...
function PreferenceRow({
  ...
  return (
    <li className="flex items-start gap-5 py-4">
      ...
      {locked ? (
        <span className="mt-0.5 shrink-0 t-caption font-semibold uppercase tracking-wide text-secondary">{alwaysOn}</span>
      ) : (
