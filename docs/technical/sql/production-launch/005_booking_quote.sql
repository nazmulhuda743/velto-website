-- Hardened website booking / quote intake for the existing Ops task list.
...
begin
  ...
  select id into v_id from public.tasks where dedupe_key=v_key;
  ...
  -- Exact matching avoids routing the form label "Outside Uttara Sectors 1-18" to RUAP.
  v_outlet := case when v_area_key='uttara sector 18' then 'RUAP' else 'S11' end;

  v_attribution := case
    when jsonb_typeof(p_payload->'attribution')='object' then p_payload->'attribution'
    else '{}'::jsonb
  end;
  select string_agg(key || '=' || left(value, 256), ', ' order by key) into v_attr
  from jsonb_each_text(v_attribution)
  where key = any(array[
    'utm_source','utm_medium','utm_campaign','utm_content','utm_term',
    'fbclid','fbc','fbp','gclid','landing_page','source','medium',
    'campaign','content','ad','service'
  ]::text[]);
  ...
  insert into public.tasks (
    title,type,priority,status,due_at,outlet_code,description,
    assigned_by_name,source,source_ref,dedupe_key
  ) values (
    v_title,
    case when p_kind='booking' then 'pickup' else 'call' end,
    'high','open',now()+interval '1 hour',v_outlet,v_desc,
