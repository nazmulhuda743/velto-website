...
- [ ] Service-page and homepage work is visually approved.
- [ ] Launch hardening is merged without changing approved visual behavior.
...
- [ ] static security audit / `npm run test:security`
...
The smoke audit verifies public routes, accessibility structure, image stability, a real 404, canonical output, sitemap, robots rules, security headers and public form request guards.
...
Current repository status: the reviewed Velto Ops gateway implementation is present. Website writes remain gated by `VELTO_OPS_WRITES_ENABLED`; the committed example keeps it `false`. Do not enable production writes until the production SQL/function, Supabase configuration and controlled end-to-end checks are ready.

Before launch:

- [ ] the production database function/migration has been reviewed and applied.
- [ ] production Supabase configuration points at the intended Ops project.
...
4. Verify the production Ops function and server configuration.
...
Velto V1 is launch-ready only when the final integrated branch is built, tested, browser-verified and connected to the real production workflow. Placeholder success, unverified pricing or an untested production write path are launch blockers.
