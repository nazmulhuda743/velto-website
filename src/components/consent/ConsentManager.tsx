 ...
 * - "Accept all" and "Reject non-essential" are equal buttons, side by side.
    ...
    <>
      {banner && !prefsOpen ? (
        <section
          ...
          className="fixed inset-x-0 bottom-0 z-[45] border-t border-line bg-white px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-5 shadow-[0_-8px_24px_rgba(0,49,83,0.10)] md:inset-x-auto md:bottom-6 md:left-6 md:w-[440px] md:rounded-lg md:border md:p-6 md:shadow-[0_12px_32px_rgba(0,49,83,0.14)]"
        >
          <h2 id={titleId} className="text-[17px] font-semibold leading-snug text-navy">
            Cookies on the Velto website
          </h2>
          <p className="mt-2 t-small text-body">
            Essential cookies keep the site and its forms working. With your permission, Velto would also like to
            measure how the site is used and how its ads perform. You can change this any time from Cookie settings in
            the footer.{" "}
          ...
          <button
            ...
            className="mt-3 inline-flex min-h-11 items-center t-small font-semibold text-navy underline underline-offset-4 hover:text-action"
          >
            Manage preferences
          </button>
        </section>
...
function ConsentButton({ children, onClick, strong = false }: { children: React.ReactNode; onClick: () => void; strong?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex h-12 items-center justify-center rounded-md px-4 text-[15px] font-semibold leading-tight transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan ${
        strong
