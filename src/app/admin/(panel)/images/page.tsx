      ...
      <nav aria-label="Pages" className="mt-6 flex flex-wrap gap-2">
        {tabs.map((t) => (
          <a
            ...
            className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 t-small font-semibold ${pageKey === t.key ? "border-navy bg-navy text-white" : "border-line bg-white text-navy hover:border-navy"}`}
          >
            {t.title}
            ...
            <li key={id} id={`slot-${id}`} className="admin-card overflow-hidden">
              ...
              <div className="p-4">
                ...
                <form action={saveImageAction} className="mt-3 space-y-3">
                  ...
                  <ImageFileInput name="image" accept="image/jpeg,image/png,image/webp,image/avif" className="block w-full t-small" aria-label={`New photo for ${slotName(id)}`} />
                  <input
                    name="alt"
